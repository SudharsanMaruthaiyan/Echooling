...
const CounterCard = ({data}) => {
  ...
  return (
    <>
        <div className=" grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 justify-center gap-16">
            {data.map((item, index) => (
                <div
                ...
                >
                    <div className=" absolute -left-6 -top-5 ">
                        <img
                            src={item.imageUrl}
                            alt={item.label}
                            className=" rounded-full bg-[#1a68f9a8] p-4 w-16 h-16 stroke-[#1A68F9] text-[#1A68F9]"
                        />
                    </div>
